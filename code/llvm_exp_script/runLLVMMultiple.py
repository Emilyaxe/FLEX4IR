...
root_path = '/data/szy/extension/LLVM-NEW-extension'
...
def random_file_prefix(prefix_name='tmp'):
    ...
    temp_filename = f"{prefix_name}.{random_str}.ll"
...
class OptRuner:
    ...
    def run_opt(self):
        ...
        origin_out_mlir = out_mlir_dir + os.sep + file_name
        report = config.result_dir + os.sep + file_name + '.crash.txt'
        ensure_directory_exists(origin_out_mlir)
        ensure_directory_exists(report)
        cmd = (' ').join([args.mlir_opt,  self.seed,  '-S', '-o', origin_out_mlir, '2>', report])
        execmd_limit_time(cmd, args.timeout)
        
        # parse failed - check if output file exists and has content
        if not os.path.exists(origin_out_mlir) or get_file_content(origin_out_mlir).strip() == '':
            # invalid mlir program, remove the out_mlir_dir and crash report
            shutil.rmtree(out_mlir_dir)
            if os.path.exists(report) and self.false_crash(report):
                os.remove(report)
            return
        
        # parse success but check if it's a false crash (no Stack dump)
        if os.path.exists(report) and self.false_crash(report):
            os.remove(report)
        ...
        for option in self.opts:
            option = option.strip()
            # result/result0/tmp.1Vp5kMxXc2.mlir/after_-canonicalize.ll
            out_mlir = out_mlir_dir + os.sep + 'after_' + option + '.ll'
            ...
            cmd = (' ').join([args.mlir_opt,  f"-passes={option}",  self.seed,  '-S', '-o', out_mlir, '2>', report])
            ...
            if os.path.exists(report) and self.false_crash(report):
                os.remove(report)
            # if out_mlir is empty, remove it
            if os.path.exists(out_mlir) and os.path.getsize(out_mlir) == 0:
                os.remove(out_mlir)
            # collect new generated mlir program
            elif os.path.exists(out_mlir) and self.is_new_generated(out_mlir, origin_out_mlir):
...
def main():
    ...
    execmd(f'rm -f {root_path}/exp_script/new_generated_pid_*')
    execmd(f'rm -f {root_path}/exp_script/correct_generated_pid_*')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--mlir_opt", type=str, help="Path of mlir_opt",
                        default=f"{llvm_dir}/llvm-release/install/opt")
    parser.add_argument("--optfile", type=str, help="Path of opt cmd file",
                        default=f"{root_path}/transform_passes.txt")
    ...
    parser.add_argument(
        '--multiprocess', type=int, default=48, 
